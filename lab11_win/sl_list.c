...
intnode_t *add(intnode_t *head, int elem, int index) {
    ...
    // If we reach NULL before the desired index, the index is invalid
    assert(current != NULL);
...
int count_in_sll(intnode_t *head, int target) {
    ...
    // Check the current node and recurse on the rest of the list
    int count = (head->value == target) ? 1 : 0;
...
int last_in_sll(intnode_t *head) {
    // Base case: If the list is empty, terminate with an assertion
    assert(head != NULL);

    // Base case: If the current node is the last node, return its value
    if (head->next == NULL) {
        return head->value;
    }
